...
if ( ! class_exists( 'Pixelgrade_Custom_Loops_For_Pages' ) ) :
...
class Pixelgrade_Custom_Loops_For_Pages {
	...
	public function init() {
		...
		add_action( 'pre_get_posts', array( $this, 'jetpack_infinite_scroll_hooks' ), 3 );
		// And for AJAX Jetpack Infinite Scroll calls
		add_action( 'custom_ajax_infinite_scroll', array( $this, 'jetpack_infinite_scroll_hooks' ), 9 );
	}
	 ...
	 * @since 1.0.0
	 */
	private function validate_page_slug() {
	...
	public function pre_get_posts( $q ) {
		...
		if ( ! is_admin() // Only target the front end
			...
			if ( ! empty( $page_ID )
				...
				// FILTERS:
				add_filter( 'found_posts', array( $this, 'found_posts' ), PHP_INT_MAX, 2 );
				...
				add_action( 'loop_end', array( $this, 'loop_end' ),   1 );
	 ...
	 */
	public function injector_query() {
	 ...
	 */
	public function found_posts( $found_posts, $q ) {
		if ( ! $q->is_main_query() )
			return $found_posts;
	 ...
	 */
	public function jetpack_infinite_scroll_hooks() {
		if ( ! $this->injector_query instanceof WP_Query )
			return;

		add_filter( 'infinite_scroll_query_object', array( $this, 'jetpack_force_injector_query' ), 10, 1 );
		add_filter( 'infinite_scroll_settings', array( $this, 'jetpack_fix_infinite_scroll_settings' ), 10, 1 );
		add_filter( 'infinite_scroll_js_settings', array( $this, 'jetpack_fix_infinite_scroll_js_settings' ), 10, 1 );
	}
	 ...
	 * @return WP_Query
	 */
	public function jetpack_force_injector_query( $query ) {
	 ...
	 * @return array
	 */
	public function jetpack_fix_infinite_scroll_settings( $settings ) {
	 ...
	 * @return array
	 */
	public function jetpack_fix_infinite_scroll_js_settings( $settings ) {
		...
		$settings['last_post_date'] = $this->get_last_post_date();
