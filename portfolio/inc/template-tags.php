		...
		if ( 'category' == $items_primary_meta || 'category' == $items_secondary_meta ) {
			...
			if ( is_page() ) {
				...
				if ( ! empty( $main_category ) ) {
					$category .= '<span class="screen-reader-text">' . esc_html__( 'Main Type', 'components_txtd' ) . '</span><ul>' . PHP_EOL;
	...
	function pixelgrade_portfolio_the_older_projects_button( $query = null ) {
		...
		if ( ! empty( $older_posts_link ) ) : ?>
			...
			<nav class="navigation posts-navigation" role="navigation">
				<h2 class="screen-reader-text"><?php esc_html_e( 'Projects navigation', 'components_txtd' ); ?></h2>
				<div class="nav-links">
					<div class="nav-previous">
						<a href="<?php echo esc_url( $older_posts_link ); ?>"><?php esc_html_e( 'Older projects', 'components_txtd' ); ?></a>
					</div>
				</div>
			</nav>
		<?php endif;
