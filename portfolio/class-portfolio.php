...
class Pixelgrade_Portfolio extends Pixelgrade_Component_Main {
	...
	public function fire_up() {
		...
		if ( ! self::site_supports_portfolio() ) {
			return;
		}
		...
		if ( ! empty( $this->config['templates'] ) ) {
			$this->templater = self::setup_custom_templates( $this->config['templates'], self::COMPONENT_SLUG, 50 );
		}
	}
	 ...
	 * @return bool
	 */
	public static function site_supports_portfolio() {
	...
	public function register_hooks() {
		...
		add_filter( 'default_content', array( $this, 'default_project_content' ), 10, 2 );

		// Add a field to the Settings > Reading page for setting the Projects Page, just like Posts Page
		add_action( 'admin_init', array( $this, 'reading_settings_fields' ) );

		// Setup the page_for_projects logic, the same way the page_for_posts works
		add_action( 'edit_form_top', array( $this, 'handle_page_for_projects_edit_page' ), 10, 1 );
		add_action( 'parse_query', array( $this, 'handle_page_for_projects_query' ), 10, 1 );
		add_filter( 'post_type_archive_link', array( $this, 'handle_page_for_projects_permalink' ), 10, 2 );
		add_filter( 'document_title_parts', array( $this, 'handle_page_for_projects_head_title' ), 10, 2 );
	 ...
	 * @return array|bool
	 */
	public function exclude_featured_projects( $page_ID ) {
		if ( ! empty( $page_ID ) ) {
	 ...
	 */
	public function default_project_content( $content, $post ) {
	...
	public function reading_settings_fields() {
		...
		if ( ! get_pages() ) {
			...
			add_settings_section(
				...
				'Projects',
				array( $this, 'reading_settings_section_callback' ),
				'reading'
			);
			...
			add_settings_field(
				'page_for_projects',
				'Projects Page',
				array( $this, 'page_for_projects_setting_callback' ),
				'reading',
				'portfolio_setting_section'
			);
	...
	//
	public function reading_settings_section_callback() {
	...
	//
	public function page_for_projects_setting_callback() {
	 ...
	 * @param WP_Post $post
	 */
	public function handle_page_for_projects_edit_page( $post ) {
		if ( $post->ID == get_option( 'page_for_projects' ) && empty( $post->post_content ) ) {
			add_action( 'edit_form_after_title', array( $this, 'projects_page_notice' ) );
	 ...
	 */
	public function handle_page_for_projects_query( $query ) {
	 ...
	 */
	public function handle_page_for_projects_permalink( $link, $post_type ) {
	 ...
	 */
	public function handle_page_for_projects_head_title( $title_parts ) {
